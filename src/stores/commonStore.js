    ...
    getServices = () => {
        try{
            runInAction( async() => {
                ...
                services.CBT.sort((a,b) => (a.heading> b.heading) ? 1 : ((b.heading > a.heading) ? -1 : 0))
                services.CC.sort((a,b) => (a.heading> b.heading) ? 1 : ((b.heading > a.heading) ? -1 : 0))
                services.CRE.sort((a,b) => (a.heading> b.heading) ? 1 : ((b.heading > a.heading) ? -1 : 0))
                services.CT.sort((a,b) => (a.heading> b.heading) ? 1 : ((b.heading > a.heading) ? -1 : 0))
                services.DR.sort((a,b) => (a.heading> b.heading) ? 1 : ((b.heading > a.heading) ? -1 : 0))
                services.EL.sort((a,b) => (a.heading> b.heading) ? 1 : ((b.heading > a.heading) ? -1 : 0))
    ...
    getClients = () => {
        try{
            runInAction( async() => {
                ...
                clients.sort((a,b) => (a.name> b.name) ? 1 : ((b.name > a.name) ? -1 : 0))
        ...
        try{
            runInAction( async() => {
                ...
                peopleTemp.forEach(element => {
                    switch(element.type){
                        case 'Seniors':
                            peopleList.Seniors.push({name: element.name ,picture: element.picture, credentials: this.credentialsToArray(element.credentials), description: element.description});
                            break;
                        case 'Associates':
                            peopleList.Associates.push({name: element.name ,picture: element.picture, credentials: this.credentialsToArray(element.credentials), description: element.description});
                            break;
                        case 'Councel':
                            peopleList.OffCouncel.push({name: element.name ,picture: element.picture, credentials: this.credentialsToArray(element.credentials), description: element.description});
                            break;
                        default:
                            peopleList.Associates.push({name: element.name ,picture: element.picture, credentials: this.credentialsToArray(element.credentials), description: element.description});
                            break;
